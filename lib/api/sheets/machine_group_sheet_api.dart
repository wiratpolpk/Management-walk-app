...
import 'package:management_walk/features/models/machine_group_model.dart';
...
''';
  ...
  static Worksheet? _machineGroupSheet;

  static Future init() async {
    try {
      final spreadsheet = await _gsheets.spreadsheet(_spreadsheetId);
      _machineGroupSheet =
          await _getWorkSheet(spreadsheet, title: 'MachineGroup');

      final firstRow = MachineGroupModel.getFields();
      _machineGroupSheet!.values.insertRow(1, firstRow);
    } catch (e) {
      print('Init Error: $e');
    }
  }
  ...
  static Future<List<MachineGroup>> getAll() async {
    if (_machineGroupSheet == null) return <MachineGroup>[];

    final machineGroups = await _machineGroupSheet!.values.map.allRows();
    return machineGroups == null
        ? <MachineGroup>[]
        : machineGroups.map(MachineGroup.fromJson).toList();
  }

  static Future<MachineGroup?> getById(int id) async {
    if (_machineGroupSheet == null) return null;

    final json =
        await _machineGroupSheet!.values.map.rowByKey(id, fromColumn: 1);
    return json == null ? null : MachineGroup.fromJson(json);
  }

  static Future<int> getRowCount() async {
    if (_machineGroupSheet == null) return 0;

    final lastRow = await _machineGroupSheet!.values.lastRow();
