...
import 'package:management_walk/features/models/section_store_group_model.dart';
import 'package:management_walk/features/models/user_model.dart';

class SectionStoreGroupSheetApi {
...
''';
  ...
  static Worksheet? _sectionStoreGroupSheet;

  static Future init() async {
    try {
      final spreadsheet = await _gsheets.spreadsheet(_spreadsheetId);
      _sectionStoreGroupSheet =
          await _getWorkSheet(spreadsheet, title: 'SectionStoreGroup');

      final firstRow = SectionStoreGroupModel.getFields();
      _sectionStoreGroupSheet!.values.insertRow(1, firstRow);
    } catch (e) {
      print('Init Error: $e');
    }
  }
  ...
  static Future<List<SectionStoreGroup>> getAll() async {
    if (_sectionStoreGroupSheet == null) return <SectionStoreGroup>[];

    final sectionStoreGroups =
        await _sectionStoreGroupSheet!.values.map.allRows();
    return sectionStoreGroups == null
        ? <SectionStoreGroup>[]
        : sectionStoreGroups.map(SectionStoreGroup.fromJson).toList();
  }

  static Future<SectionStoreGroup?> getById(int id) async {
    if (_sectionStoreGroupSheet == null) return null;

    final json =
        await _sectionStoreGroupSheet!.values.map.rowByKey(id, fromColumn: 1);
    return json == null ? null : SectionStoreGroup.fromJson(json);
  }

  static Future<int> getRowCount() async {
    if (_sectionStoreGroupSheet == null) return 0;

    final lastRow = await _sectionStoreGroupSheet!.values.lastRow();
