import 'package:gsheets/gsheets.dart';
import 'package:management_walk/features/models/report1_model.dart';
import 'package:management_walk/features/models/user_model.dart';

import '../../features/models/report4_model.dart';

class Report4SheetsApi {
  static const _credentials = r'''
{
  "type": "service_account",
  "project_id": "management-walk",
  "private_key_id": "658b13832f06ee96437c2eb918f143c2d50f40b2",
  "private_key": "-----BEGIN PRIVATE KEY-----\\n-----END PRIVATE KEY-----\n",
  "client_email": "gsheets@management-walk.iam.gserviceaccount.com",
  "client_id": "100624676121701366083",
  "auth_uri": "https://accounts.google.com/o/oauth2/auth",
  "token_uri": "https://oauth2.googleapis.com/token",
  "auth_provider_x509_cert_url": "https://www.googleapis.com/oauth2/v1/certs",
  "client_x509_cert_url": "https://www.googleapis.com/robot/v1/metadata/x509/gsheets%40management-walk.iam.gserviceaccount.com"
}
''';
  static final _spreadsheetId = '16WQ_uAG7lOAbK1oKVD6r7GyqEhaV5fg78QJ93rEFQRc';
  static final _gsheets = GSheets(_credentials);
  static Worksheet? _report4Sheet;

  static Future init() async {
    try {
      final spreadsheet = await _gsheets.spreadsheet(_spreadsheetId);
      _report4Sheet = await _getWorkSheet(spreadsheet, title: 'Report4');

      // final firstRow = Report4Model.getFields();
      // _report4Sheet!.values.insertRow(1, firstRow);
    } catch (e) {
      print('Init Error: $e');
    }
  }

  static Future<Worksheet> _getWorkSheet(
    Spreadsheet spreadsheet, {
    required String title,
  }) async {
    try {
      return await spreadsheet.addWorksheet(title);
    } catch (e) {
      return spreadsheet.worksheetByTitle(title)!;
    }
  }

  // static Future<List<Report4>> getAll() async {
  //   if (_report4Sheet == null) return <Report4>[];

  //   final report4s = await _report4Sheet!.values.map.allRows();
  //   return report4s == null
  //       ? <Report4>[]
  //       : report4s.map(Report4.fromJson).toList();
  // }

  // static Future<Report4?> getById(int id) async {
  //   if (_report4Sheet == null) return null;

  //   final json = await _report4Sheet!.values.map.rowByKey(id, fromColumn: 1);
  //   return json == null ? null : Report4.fromJson(json);
  // }

  static Future<int> getRowCount() async {
    if (_report4Sheet == null) return 0;

    final lastRow = await _report4Sheet!.values.lastRow();
    return lastRow == null ? 0 : int.tryParse(lastRow.first) ?? 0;
  }

  static Future insert(List<Map<String, dynamic>> rowList) async {
    if (_report4Sheet == null) return;

    _report4Sheet!.values.map.appendRows(rowList);
  }

  static Future<bool> update(
    int id,
    Map<String, dynamic> user,
  ) async {
    if (_report4Sheet == null) return false;

    return _report4Sheet!.values.map.insertRowByKey(id, user);
  }

  static Future<bool> updateCell({
    required int id,
    required String key,
    required dynamic value,
  }) async {
    if (_report4Sheet == null) return false;

    return _report4Sheet!.values
        .insertValueByKeys(value, columnKey: key, rowKey: id);
  }

  static Future<bool> deleteById(int id) async {
    if (_report4Sheet == null) return false;

    final index = await _report4Sheet!.values.rowIndexOf(id);
    if (index == -1) return false;

    return _report4Sheet!.deleteRow(index);
  }
}
