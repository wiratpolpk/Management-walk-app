...
import '../../features/models/report7_model.dart';

class Report7SheetsApi {
...
''';
  ...
  static Worksheet? _report7Sheet;

  static Future init() async {
    try {
      final spreadsheet = await _gsheets.spreadsheet(_spreadsheetId);
      _report7Sheet = await _getWorkSheet(spreadsheet, title: 'Report7');

      // final firstRow = Report7Model.getFields();
      // _report7Sheet!.values.insertRow(1, firstRow);
    } catch (e) {
      print('Init Error: $e');
    }
  }
  ...
  // static Future<List<Report7>> getAll() async {
  //   if (_report7Sheet == null) return <Report7>[];

  //   final report7s = await _report7Sheet!.values.map.allRows();
  //   return report7s == null
  //       ? <Report7>[]
  //       : report7s.map(Report7.fromJson).toList();
  // }

  // static Future<Report7?> getById(int id) async {
  //   if (_report7Sheet == null) return null;

  //   final json = await _report7Sheet!.values.map.rowByKey(id, fromColumn: 1);
  //   return json == null ? null : Report7.fromJson(json);
  // }

  static Future<int> getRowCount() async {
    if (_report7Sheet == null) return 0;

    final lastRow = await _report7Sheet!.values.lastRow();
    return lastRow == null ? 0 : int.tryParse(lastRow.first) ?? 0;
  }

  static Future insert(List<Map<String, dynamic>> rowList) async {
    if (_report7Sheet == null) return;

    _report7Sheet!.values.map.appendRows(rowList);
  }
  ...
  static Future<bool> update(
    ...
    if (_report7Sheet == null) return false;

    return _report7Sheet!.values.map.insertRowByKey(id, user);
  }
  ...
  static Future<bool> deleteById(int id) async {
    ...
    final index = await _report7Sheet!.values.rowIndexOf(id);
    if (index == -1) return false;

    return _report7Sheet!.deleteRow(index);
  }
}
