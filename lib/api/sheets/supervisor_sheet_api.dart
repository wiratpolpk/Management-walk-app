...
''';
  ...
  static Worksheet? _supervisorSheet;

  static Future init() async {
    try {
      final spreadsheet = await _gsheets.spreadsheet(_spreadsheetId);
      _supervisorSheet = await _getWorkSheet(spreadsheet, title: 'Supervisor');

      // final firstRow = SupervisorModel.getFields();
      // _supervisorSheet!.values.insertRow(1, firstRow);
    } catch (e) {
      print('Init Error: $e');
    }
  }
  ...
  // static Future<List<Supervisor>> getAll() async {
  //   if (_supervisorSheet == null) return <Supervisor>[];

  //   final users = await _supervisorSheet!.values.map.allRows();
  //   return users == null
  //       ? <Supervisor>[]
  //       : users.map(Supervisor.fromJson).toList();
  // }

  // static Future<Supervisor?> getById(int id) async {
  //   if (_supervisorSheet == null) return null;

  //   final json = await _supervisorSheet!.values.map.rowByKey(id, fromColumn: 1);
  //   return json == null ? null : Supervisor.fromJson(json);
  // }

  static Future<int> getRowCount() async {
    if (_supervisorSheet == null) return 0;

    final lastRow = await _supervisorSheet!.values.lastRow();
