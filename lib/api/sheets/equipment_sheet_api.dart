...
''';
  ...
  static Worksheet? _equipmentGroupSheet;

  static Future init() async {
    try {
      final spreadsheet = await _gsheets.spreadsheet(_spreadsheetId);
      _equipmentGroupSheet =
          await _getWorkSheet(spreadsheet, title: 'EquipmentGroup');

      final firstRow = EquipmentGroupModel.getFields();
      _equipmentGroupSheet!.values.insertRow(1, firstRow);
    } catch (e) {
      print('Init Error: $e');
    }
  }
  ...
  static Future<List<EquipmentGroup>> getAll() async {
    if (_equipmentGroupSheet == null) return <EquipmentGroup>[];

    final equipmentGroups = await _equipmentGroupSheet!.values.map.allRows();
    return equipmentGroups == null
        ? <EquipmentGroup>[]
        : equipmentGroups.map(EquipmentGroup.fromJson).toList();
  }

  static Future<EquipmentGroup?> getById(int id) async {
    if (_equipmentGroupSheet == null) return null;

    final json =
        await _equipmentGroupSheet!.values.map.rowByKey(id, fromColumn: 1);
    return json == null ? null : EquipmentGroup.fromJson(json);
  }

  static Future<int> getRowCount() async {
    if (_equipmentGroupSheet == null) return 0;

    final lastRow = await _equipmentGroupSheet!.values.lastRow();
