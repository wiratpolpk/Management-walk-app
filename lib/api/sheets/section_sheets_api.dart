...
''';
  ...
  static Worksheet? _sectionSheet;

  static Future init() async {
    try {
      final spreadsheet = await _gsheets.spreadsheet(_spreadsheetId);
      _sectionSheet = await _getWorkSheet(spreadsheet, title: 'Section');

      final firstRow = SectionModel.getFields();
      _sectionSheet!.values.insertRow(1, firstRow);
    } catch (e) {
      print('Init Error: $e');
    }
  }
  ...
  static Future<List<Section>> getAll() async {
    if (_sectionSheet == null) return <Section>[];

    final users = await _sectionSheet!.values.map.allRows();
    return users == null ? <Section>[] : users.map(Section.fromJson).toList();
  }

  static Future<Section?> getById(int id) async {
    if (_sectionSheet == null) return null;

    final json = await _sectionSheet!.values.map.rowByKey(id, fromColumn: 1);
    return json == null ? null : Section.fromJson(json);
  }

  static Future<int> getRowCount() async {
    if (_sectionSheet == null) return 0;

    final lastRow = await _sectionSheet!.values.lastRow();
