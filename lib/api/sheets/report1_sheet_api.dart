...
''';
  ...
  static Worksheet? _report1Sheet;

  static Future init() async {
    try {
      final spreadsheet = await _gsheets.spreadsheet(_spreadsheetId);
      _report1Sheet = await _getWorkSheet(spreadsheet, title: 'Report1');

      // final firstRow = Report1Model.getFields();
      // _report1Sheet!.values.insertRow(1, firstRow);
    } catch (e) {
      print('Init Error: $e');
    }
  }
  ...
  // static Future<List<Report1>> getAll() async {
  //   if (_report1Sheet == null) return <Report1>[];

  //   final report1s = await _report1Sheet!.values.map.allRows();
  //   return report1s == null
  //       ? <Report1>[]
  //       : report1s.map(Report1.fromJson).toList();
  // }

  // static Future<Report1?> getById(int id) async {
  //   if (_report1Sheet == null) return null;

  //   final json = await _report1Sheet!.values.map.rowByKey(id, fromColumn: 1);
  //   return json == null ? null : Report1.fromJson(json);
  // }

  static Future<int> getRowCount() async {
    if (_report1Sheet == null) return 0;

    final lastRow = await _report1Sheet!.values.lastRow();
    return lastRow == null ? 0 : int.tryParse(lastRow.first) ?? 0;
  }

  static Future insert(List<Map<String, dynamic>> rowList) async {
    if (_report1Sheet == null) return;

    _report1Sheet!.values.map.appendRows(rowList);
  }
  ...
  static Future<bool> update(
    ...
    if (_report1Sheet == null) return false;

    return _report1Sheet!.values.map.insertRowByKey(id, user);
  }
  ...
  static Future<bool> deleteById(int id) async {
    ...
    final index = await _report1Sheet!.values.rowIndexOf(id);
    if (index == -1) return false;

    return _report1Sheet!.deleteRow(index);
  }
}
