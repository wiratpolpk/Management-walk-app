import 'package:gsheets/gsheets.dart';
import 'package:management_walk/features/models/equipment_model.dart';
import 'package:management_walk/features/models/section_model.dart';
import 'package:management_walk/features/models/user_model.dart';

import '../../features/models/safety_group_model.dart';

class SafetyGroupSheetApi {
  static const _credentials = r'''
{
  "type": "service_account",
  "project_id": "management-walk",
  "private_key_id": "658b13832f06ee96437c2eb918f143c2d50f40b2",
  "private_key": "-----BEGIN PRIVATE KEY-----\\n-----END PRIVATE KEY-----\n",
  "client_email": "gsheets@management-walk.iam.gserviceaccount.com",
  "client_id": "100624676121701366083",
  "auth_uri": "https://accounts.google.com/o/oauth2/auth",
  "token_uri": "https://oauth2.googleapis.com/token",
  "auth_provider_x509_cert_url": "https://www.googleapis.com/oauth2/v1/certs",
  "client_x509_cert_url": "https://www.googleapis.com/robot/v1/metadata/x509/gsheets%40management-walk.iam.gserviceaccount.com"
}
''';
  static final _spreadsheetId = '16WQ_uAG7lOAbK1oKVD6r7GyqEhaV5fg78QJ93rEFQRc';
  static final _gsheets = GSheets(_credentials);
  static Worksheet? _safetyGroupSheet;

  static Future init() async {
    try {
      final spreadsheet = await _gsheets.spreadsheet(_spreadsheetId);
      _safetyGroupSheet =
          await _getWorkSheet(spreadsheet, title: 'SafetyGroup');

      final firstRow = SafetyGroupModel.getFields();
      _safetyGroupSheet!.values.insertRow(1, firstRow);
    } catch (e) {
      print('Init Error: $e');
    }
  }

  static Future<Worksheet> _getWorkSheet(
    Spreadsheet spreadsheet, {
    required String title,
  }) async {
    try {
      return await spreadsheet.addWorksheet(title);
    } catch (e) {
      return spreadsheet.worksheetByTitle(title)!;
    }
  }

  static Future<List<SafetyGroup>> getAll() async {
    if (_safetyGroupSheet == null) return <SafetyGroup>[];

    final safetyGroups = await _safetyGroupSheet!.values.map.allRows();
    return safetyGroups == null
        ? <SafetyGroup>[]
        : safetyGroups.map(SafetyGroup.fromJson).toList();
  }

  static Future<SafetyGroup?> getById(int id) async {
    if (_safetyGroupSheet == null) return null;

    final json =
        await _safetyGroupSheet!.values.map.rowByKey(id, fromColumn: 1);
    return json == null ? null : SafetyGroup.fromJson(json);
  }

  static Future<int> getRowCount() async {
    if (_safetyGroupSheet == null) return 0;

    final lastRow = await _safetyGroupSheet!.values.lastRow();
    return lastRow == null ? 0 : int.tryParse(lastRow.first) ?? 0;
  }
}
