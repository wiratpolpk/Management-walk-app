...
''';
  ...
  static Worksheet? _keyForSearchSheet;

  static Future init() async {
    try {
      final spreadsheet = await _gsheets.spreadsheet(_spreadsheetId);
      _keyForSearchSheet =
          await _getWorkSheet(spreadsheet, title: 'KeyForSearch');

      final firstRow = KeyForSearchModel.getFields();
      _keyForSearchSheet!.values.insertRow(1, firstRow);
    } catch (e) {
      print('Init Error: $e');
    }
  }
  ...
  static Future<List<KeyForSearch>> getAll() async {
    if (_keyForSearchSheet == null) return <KeyForSearch>[];

    final keyForSearchs = await _keyForSearchSheet!.values.map.allRows();
    return keyForSearchs == null
        ? <KeyForSearch>[]
        : keyForSearchs.map(KeyForSearch.fromJson).toList();
  }

  static Future<KeyForSearch?> getById(String id) async {
    if (_keyForSearchSheet == null) return null;

    final json =
        await _keyForSearchSheet!.values.map.rowByKey(id, fromColumn: 1);
    return json == null ? null : KeyForSearch.fromJson(json);
  }

  static Future<int> getRowCount() async {
    if (_keyForSearchSheet == null) return 0;

    final lastRow = await _keyForSearchSheet!.values.lastRow();
    return lastRow == null ? 0 : int.tryParse(lastRow.first) ?? 0;
  }

  static Future insert(List<Map<String, dynamic>> rowList) async {
    if (_keyForSearchSheet == null) return;

    _keyForSearchSheet!.values.map.appendRows(rowList);
  }
  ...
  static Future<bool> update(
    ...
    if (_keyForSearchSheet == null) return false;

    return _keyForSearchSheet!.values.map.insertRowByKey(id, user);
  }
  ...
  static Future<bool> updateCell({
    ...
    return _keyForSearchSheet!.values
  ...
  static Future<bool> deleteById(String id) async {
    ...
    final index = await _keyForSearchSheet!.values.rowIndexOf(id);
    if (index == -1) return false;

    return _keyForSearchSheet!.deleteRow(index);
  }
}
