...
import '../../features/models/management_group_model.dart';

class ManagementGroupSheetApi {
...
''';
  ...
  static Worksheet? _managementGroupSheet;

  static Future init() async {
    try {
      final spreadsheet = await _gsheets.spreadsheet(_spreadsheetId);
      _managementGroupSheet =
          await _getWorkSheet(spreadsheet, title: 'ManagementGroup');

      final firstRow = ManagementGroupModel.getFields();
      _managementGroupSheet!.values.insertRow(1, firstRow);
    } catch (e) {
      print('Init Error: $e');
    }
  }
  ...
  static Future<List<ManagementGroup>> getAll() async {
    if (_managementGroupSheet == null) return <ManagementGroup>[];

    final managementGroups = await _managementGroupSheet!.values.map.allRows();
    return managementGroups == null
        ? <ManagementGroup>[]
        : managementGroups.map(ManagementGroup.fromJson).toList();
  }

  static Future<ManagementGroup?> getById(int id) async {
    if (_managementGroupSheet == null) return null;

    final json =
        await _managementGroupSheet!.values.map.rowByKey(id, fromColumn: 1);
    return json == null ? null : ManagementGroup.fromJson(json);
  }

  static Future<int> getRowCount() async {
    if (_managementGroupSheet == null) return 0;

    final lastRow = await _managementGroupSheet!.values.lastRow();
