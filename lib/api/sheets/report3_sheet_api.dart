...
import '../../features/models/report3_model.dart';

class Report3SheetsApi {
...
''';
  ...
  static Worksheet? _report3Sheet;

  static Future init() async {
    try {
      final spreadsheet = await _gsheets.spreadsheet(_spreadsheetId);
      _report3Sheet = await _getWorkSheet(spreadsheet, title: 'Report3');

      // final firstRow = Report3Model.getFields();
      // _report3Sheet!.values.insertRow(1, firstRow);
    } catch (e) {
      print('Init Error: $e');
    }
  }
  ...
  // static Future<List<Report3>> getAll() async {
  //   if (_report3Sheet == null) return <Report3>[];

  //   final report3s = await _report3Sheet!.values.map.allRows();
  //   return report3s == null
  //       ? <Report3>[]
  //       : report3s.map(Report3.fromJson).toList();
  // }

  // static Future<Report3?> getById(int id) async {
  //   if (_report3Sheet == null) return null;

  //   final json = await _report3Sheet!.values.map.rowByKey(id, fromColumn: 1);
  //   return json == null ? null : Report3.fromJson(json);
  // }

  static Future<int> getRowCount() async {
    if (_report3Sheet == null) return 0;

    final lastRow = await _report3Sheet!.values.lastRow();
    return lastRow == null ? 0 : int.tryParse(lastRow.first) ?? 0;
  }

  static Future insert(List<Map<String, dynamic>> rowList) async {
    if (_report3Sheet == null) return;

    _report3Sheet!.values.map.appendRows(rowList);
  }
  ...
  static Future<bool> update(
    ...
    if (_report3Sheet == null) return false;

    return _report3Sheet!.values.map.insertRowByKey(id, user);
  }
  ...
  static Future<bool> deleteById(int id) async {
    ...
    final index = await _report3Sheet!.values.rowIndexOf(id);
    if (index == -1) return false;

    return _report3Sheet!.deleteRow(index);
  }
}
