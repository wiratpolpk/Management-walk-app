...
import '../../features/models/report2_model.dart';

class Report2SheetsApi {
...
''';
  ...
  static Worksheet? _report2Sheet;

  static Future init() async {
    try {
      final spreadsheet = await _gsheets.spreadsheet(_spreadsheetId);
      _report2Sheet = await _getWorkSheet(spreadsheet, title: 'Report2');

      // final firstRow = Report2Model.getFields();
      // _report2Sheet!.values.insertRow(1, firstRow);
    } catch (e) {
      print('Init Error: $e');
    }
  }
  ...
  // static Future<List<Report2>> getAll() async {
  //   if (_report2Sheet == null) return <Report2>[];

  //   final report2s = await _report2Sheet!.values.map.allRows();
  //   return report2s == null
  //       ? <Report2>[]
  //       : report2s.map(Report2.fromJson).toList();
  // }

  // static Future<Report2?> getById(int id) async {
  //   if (_report2Sheet == null) return null;

  //   final json = await _report2Sheet!.values.map.rowByKey(id, fromColumn: 1);
  //   return json == null ? null : Report2.fromJson(json);
  // }

  static Future<int> getRowCount() async {
    if (_report2Sheet == null) return 0;

    final lastRow = await _report2Sheet!.values.lastRow();
    return lastRow == null ? 0 : int.tryParse(lastRow.first) ?? 0;
  }

  static Future insert(List<Map<String, dynamic>> rowList) async {
    if (_report2Sheet == null) return;

    _report2Sheet!.values.map.appendRows(rowList);
  }
  ...
  static Future<bool> update(
    ...
    if (_report2Sheet == null) return false;

    return _report2Sheet!.values.map.insertRowByKey(id, user);
  }
  ...
  static Future<bool> deleteById(int id) async {
    ...
    final index = await _report2Sheet!.values.rowIndexOf(id);
    if (index == -1) return false;

    return _report2Sheet!.deleteRow(index);
  }
}
