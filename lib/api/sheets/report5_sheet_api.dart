...
import '../../features/models/report5_model.dart';

class Report5SheetsApi {
...
''';
  ...
  static Worksheet? _report5Sheet;

  static Future init() async {
    try {
      final spreadsheet = await _gsheets.spreadsheet(_spreadsheetId);
      _report5Sheet = await _getWorkSheet(spreadsheet, title: 'Report5');

      // final firstRow = Report5Model.getFields();
      // _report5Sheet!.values.insertRow(1, firstRow);
    } catch (e) {
      print('Init Error: $e');
    }
  }
  ...
  // static Future<List<Report5>> getAll() async {
  //   if (_report5Sheet == null) return <Report5>[];

  //   final report5s = await _report5Sheet!.values.map.allRows();
  //   return report5s == null
  //       ? <Report5>[]
  //       : report5s.map(Report5.fromJson).toList();
  // }

  // static Future<Report5?> getById(int id) async {
  //   if (_report5Sheet == null) return null;

  //   final json = await _report5Sheet!.values.map.rowByKey(id, fromColumn: 1);
  //   return json == null ? null : Report5.fromJson(json);
  // }

  static Future<int> getRowCount() async {
    if (_report5Sheet == null) return 0;

    final lastRow = await _report5Sheet!.values.lastRow();
    return lastRow == null ? 0 : int.tryParse(lastRow.first) ?? 0;
  }

  static Future insert(List<Map<String, dynamic>> rowList) async {
    if (_report5Sheet == null) return;

    _report5Sheet!.values.map.appendRows(rowList);
  }
  ...
  static Future<bool> update(
    ...
    if (_report5Sheet == null) return false;

    return _report5Sheet!.values.map.insertRowByKey(id, user);
  }
  ...
  static Future<bool> deleteById(int id) async {
    ...
    final index = await _report5Sheet!.values.rowIndexOf(id);
    if (index == -1) return false;

    return _report5Sheet!.deleteRow(index);
  }
}
