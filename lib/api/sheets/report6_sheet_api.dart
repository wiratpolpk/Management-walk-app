...
import '../../features/models/report6_model.dart';

class Report6SheetsApi {
...
''';
  ...
  static Worksheet? _report6Sheet;

  static Future init() async {
    try {
      final spreadsheet = await _gsheets.spreadsheet(_spreadsheetId);
      _report6Sheet = await _getWorkSheet(spreadsheet, title: 'Report6');

      // final firstRow = Report6Model.getFields();
      // _report6Sheet!.values.insertRow(1, firstRow);
    } catch (e) {
      print('Init Error: $e');
    }
  }
  ...
  // static Future<List<Report6>> getAll() async {
  //   if (_report6Sheet == null) return <Report6>[];

  //   final report6s = await _report6Sheet!.values.map.allRows();
  //   return report6s == null
  //       ? <Report6>[]
  //       : report6s.map(Report6.fromJson).toList();
  // }

  // static Future<Report6?> getById(int id) async {
  //   if (_report6Sheet == null) return null;

  //   final json = await _report6Sheet!.values.map.rowByKey(id, fromColumn: 1);
  //   return json == null ? null : Report6.fromJson(json);
  // }

  static Future<int> getRowCount() async {
    if (_report6Sheet == null) return 0;

    final lastRow = await _report6Sheet!.values.lastRow();
    return lastRow == null ? 0 : int.tryParse(lastRow.first) ?? 0;
  }

  static Future insert(List<Map<String, dynamic>> rowList) async {
    if (_report6Sheet == null) return;

    _report6Sheet!.values.map.appendRows(rowList);
  }
  ...
  static Future<bool> update(
    ...
    if (_report6Sheet == null) return false;

    return _report6Sheet!.values.map.insertRowByKey(id, user);
  }
  ...
  static Future<bool> deleteById(int id) async {
    ...
    final index = await _report6Sheet!.values.rowIndexOf(id);
    if (index == -1) return false;

    return _report6Sheet!.deleteRow(index);
  }
}
