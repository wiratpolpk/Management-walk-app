...
import '../../features/models/report8_model.dart';

class Report8SheetsApi {
...
''';
  ...
  static Worksheet? _report8Sheet;

  static Future init() async {
    try {
      final spreadsheet = await _gsheets.spreadsheet(_spreadsheetId);
      _report8Sheet = await _getWorkSheet(spreadsheet, title: 'Report8');

      // final firstRow = Report8Model.getFields();
      // _report8Sheet!.values.insertRow(1, firstRow);
    } catch (e) {
      print('Init Error: $e');
    }
  }
  ...
  // static Future<List<Report8>> getAll() async {
  //   if (_report8Sheet == null) return <Report8>[];

  //   final report8s = await _report8Sheet!.values.map.allRows();
  //   return report8s == null
  //       ? <Report8>[]
  //       : report8s.map(Report8.fromJson).toList();
  // }

  // static Future<Report8?> getById(int id) async {
  //   if (_report8Sheet == null) return null;

  //   final json = await _report8Sheet!.values.map.rowByKey(id, fromColumn: 1);
  //   return json == null ? null : Report8.fromJson(json);
  // }

  static Future<int> getRowCount() async {
    if (_report8Sheet == null) return 0;

    final lastRow = await _report8Sheet!.values.lastRow();
    return lastRow == null ? 0 : int.tryParse(lastRow.first) ?? 0;
  }

  static Future insert(List<Map<String, dynamic>> rowList) async {
    if (_report8Sheet == null) return;

    _report8Sheet!.values.map.appendRows(rowList);
  }
  ...
  static Future<bool> update(
    ...
    if (_report8Sheet == null) return false;

    return _report8Sheet!.values.map.insertRowByKey(id, user);
  }
  ...
  static Future<bool> deleteById(int id) async {
    ...
    final index = await _report8Sheet!.values.rowIndexOf(id);
    if (index == -1) return false;

    return _report8Sheet!.deleteRow(index);
  }
}
