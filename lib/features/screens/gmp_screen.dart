...
import 'package:management_walk/features/models/count_gmpDone_model.dart';
...
import '../models/gmpGroup_model.dart';
...
class GMPScreen extends StatefulWidget {
  const GMPScreen({Key? key}) : super(key: key);

  @override
  State<GMPScreen> createState() => _GMPScreenState();
}

class _GMPScreenState extends State<GMPScreen> {
  var gmpID = <int?>[];
  var gmpDescs = <String>[];
  var yesChecked = <bool>[];
  var noChecked = <bool>[];
  ...
  void initState() {
    ...
    fetchGmpGroup().then((value) async {
      for (int i = 0; i < value.length; i++) {
        ...
        setState(() {
          gmpID.add(int.parse(value[i].questionID!));
          gmpDescs.add(value[i].questionDesc!);
      ...
      getGmpDone().then(
        (value) {
          countComplete = value[0].countGmpDone!;

          if (countComplete == gmpDescs.length) {
            isCanClick = true.obs;
          } else {
  ...
  Future<List<GmpModel>> fetchGmpGroup() async {
    SharedPreferences prefs = await SharedPreferences.getInstance();
    ...
    if (langaugeText == "TH") {
      final response = await http.get(Uri.parse(API.getGmpGroup));

      if (response.statusCode == 200) {
        final List result = json.decode(response.body);
        return result.map((e) => GmpModel.fromJson(e)).toList();
      } else {
        throw Exception('Failed to load data');
      }
    } else {
      final response = await http.get(Uri.parse(API.getGmpGroupEN));
  ...
  Future<List<GmpDoneModel>> getGmpDone() async {
    try {
      final response = await http.get(Uri.parse(API.countGmpDone));
      if (200 == response.statusCode) {
        List<GmpDoneModel> list = parseResponse(response.body);
        return list;
      } else {
        return <GmpDoneModel>[];
      }
  ...
  static List<GmpDoneModel> parseResponse(String responseBody) {
    final parsed = json.decode(responseBody).cast<Map<String, dynamic>>();
    return parsed
        .map<GmpDoneModel>((json) => GmpDoneModel.fromJson(json))
        .toList();
  }
  ...
  Future updateGmpDone(String questionID) async {
    final response = await http.post(
      Uri.parse(API.updateGmpDone),
      body: {
                                ...
                                Expanded(
                                  ...
                                  child: CheckboxListTile(
                                    onChanged: (value) async {
                                      var id = '$idtemp${gmpID[index]}';
                                      var qID = gmpID[index].toString();
                                      ...
                                      await updateGmpDone(qID);

                                      await getGmpDone().then(
                                        (value) {
                                          // print(
                                          //     'value: ${value[0].countEquipmentDone}');

                                          countComplete =
                                              value[0].countGmpDone!;
                ...
                child: Obx(() => ElevatedButton(
                      ...
                      onPressed: () async {
                        if (isCanClick == true.obs) {
                          ...
                          prefs.setString('gmpDone', "completed");
