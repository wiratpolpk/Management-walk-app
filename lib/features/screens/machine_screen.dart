...
import 'package:management_walk/features/models/count_machineDone_model.dart';
...
import '../models/machineGroup_model.dart';
...
class _MachineScreenState extends State<MachineScreen> {
  ...
  void initState() {
    ...
    fetchMachineGroup().then((value) async {
      for (int i = 0; i < value.length; i++) {
        ...
        setState(() {
          machineID.add(int.parse(value[i].questionID!));
          machineDescs.add(value[i].questionDesc!);

          if (mwYesCheck == "1") {
            yesChecked.add(true);
            updateMachineDone(qID);
            // countInt.add(1);
          } else {
      ...
      getMachineDone().then(
        (value) {
          countComplete = value[0].countMachineDone!;

          if (countComplete == machineDescs.length) {
            isCanClick = true.obs;
          } else {
  ...
  Future initIDTemp() async {
    ...
    if ((prefs.getString('machineDone')) == "completed") {
      isCanClick = true.obs;
    } else {
      isCanClick = false.obs;
    }
  ...
  Future<List<MachineModel>> fetchMachineGroup() async {
    SharedPreferences prefs = await SharedPreferences.getInstance();
    ...
    if (langaugeText == "TH") {
      final response = await http.get(Uri.parse(API.getMachineGroup));

      if (response.statusCode == 200) {
        final List result = json.decode(response.body);
        return result.map((e) => MachineModel.fromJson(e)).toList();
      } else {
        throw Exception('Failed to load data');
      }
    } else {
      final response = await http.get(Uri.parse(API.getMachineGroupEN));
  ...
  Future<List<MachineDoneModel>> getMachineDone() async {
    try {
      final response = await http.get(Uri.parse(API.countMachineDone));
      if (200 == response.statusCode) {
        List<MachineDoneModel> list = parseResponse(response.body);
        return list;
      } else {
        return <MachineDoneModel>[];
      }
    } catch (e) {
      return <MachineDoneModel>[];
    }
  ...
  static List<MachineDoneModel> parseResponse(String responseBody) {
    final parsed = json.decode(responseBody).cast<Map<String, dynamic>>();
    return parsed
        .map<MachineDoneModel>((json) => MachineDoneModel.fromJson(json))
        .toList();
  }
  ...
  Future updateMachineDone(String questionID) async {
    final response = await http.post(
      Uri.parse(API.updateMachineDone),
      body: {
                                ...
                                Expanded(
                                  ...
                                  child: CheckboxListTile(
                                    onChanged: (value) async {
                                      var id = '$idtemp${machineID[index]}';
                                      var qID = machineID[index].toString();
                                      ...
                                      await updateMachineDone(qID);

                                      await getMachineDone().then(
                                        (value) {
                                          // print(
                                          //     'value: ${value[0].countEquipmentDone}');

                                          countComplete =
                                              value[0].countMachineDone!;
                ...
                child: Obx(() => ElevatedButton(
                      ...
                      onPressed: () async {
                        if (isCanClick == true.obs) {
                          ...
                          prefs.setString('machineDone', "completed");
