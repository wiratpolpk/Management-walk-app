...
import 'package:management_walk/features/models/count_equipmentDone_model.dart';
import 'package:management_walk/features/models/equipmentGroup_model.dart';
...
class _EquipmentState extends State<Equipment> {
  ...
  void initState() {
    ...
    fetchEquipmentGroup().then((value) async {
      for (int i = 0; i < value.length; i++) {
        ...
        setState(() {
          equipmentID.add(int.parse(value[i].questionID!));
          equipmentDescs.add(value[i].questionDesc!);

          if (mwYesCheck == "1") {
            yesChecked.add(true);
            updateEquipmentDone(qID);
            // countInt.add(1);
          } else {
      ...
      getEquipmentDone().then(
        (value) {
          // print('value: ${value[0].countEquipmentDone}');

          countComplete = value[0].countEquipmentDone!;

          if (countComplete == equipmentDescs.length) {
            isCanClick = true.obs;
          } else {
  ...
  Future initIDTemp() async {
    ...
    if ((prefs.getString('equipmentDone')) == "completed") {
      isCanClick = true.obs;
    } else {
      isCanClick = false.obs;
    }
  ...
  Future<List<EquipmentModel>> fetchEquipmentGroup() async {
    SharedPreferences prefs = await SharedPreferences.getInstance();
    ...
    if (langaugeText == "TH") {
      final response = await http.get(Uri.parse(API.getEquipmentGroup));
      ...
      final response = await http.get(Uri.parse(API.getEquipmentGroupEN));
      if (response.statusCode == 200) {
        final List result = json.decode(response.body);
        return result.map((e) => EquipmentModel.fromJson(e)).toList();
      } else {
  ...
  Future<List<EquipmentDoneModel>> getEquipmentDone() async {
    try {
      final response = await http.get(Uri.parse(API.countEquipmentDone));
      if (200 == response.statusCode) {
        List<EquipmentDoneModel> list = parseResponse(response.body);
        return list;
      } else {
        return <EquipmentDoneModel>[];
      }
    } catch (e) {
      return <EquipmentDoneModel>[];
    }
  ...
  static List<EquipmentDoneModel> parseResponse(String responseBody) {
    final parsed = json.decode(responseBody).cast<Map<String, dynamic>>();
    return parsed
        .map<EquipmentDoneModel>((json) => EquipmentDoneModel.fromJson(json))
        .toList();
  }
  ...
  Future updateEquipmentDone(String questionID) async {
    final response = await http.post(
      Uri.parse(API.updateEquipmentDone),
      body: {
        ...
        child: Scaffold(
          ...
          appBar: AppBar(
            ...
            title: Text(langaugeText == "TH".obs
                ? equipmentHeaderTextTH
                : equipmentHeaderTextEN),
            centerTitle: true,
          ),
          body: Column(
            children: [
                                        ...
                                        onPressed: () async {
                                          setState(() {
                                            commentIsSelected[index] =
                                                !commentIsSelected[index];
                                            showToast(commentIsSelected[index],
                                                index);
                                          });
                                        },
                                ...
                                Expanded(
                                  ...
                                  child: CheckboxListTile(
                                    onChanged: (value) async {
                                      ...
                                      var qID = equipmentID[index].toString();
                                      ...
                                      await updateEquipmentDone(qID);

                                      await getEquipmentDone().then(
                                        (value) {
                                          // print(
                                          //     'value: ${value[0].countEquipmentDone}');

                                          countComplete =
                                              value[0].countEquipmentDone!;

                                          if (countComplete ==
                                              equipmentDescs.length) {
                                            isCanClick = true.obs;
                                          } else {
                                            isCanClick = false.obs;
                                          }
                ...
                child: Obx(() => ElevatedButton(
                      ...
                      onPressed: () async {
                        if (isCanClick == true.obs) {
                          ...
                          prefs.setString('equipmentDone', "completed");
