...
import 'package:management_walk/features/models/count_safetyDone_model.dart';
...
import '../models/safetyGroup_model.dart';
...
class _SafetyScreenState extends State<SafetyScreen> {
  ...
  void initState() {
    ...
    fetchSafetyGroup().then((value) async {
      for (int i = 0; i < value.length; i++) {
        ...
        setState(() {
          safetyID.add(int.parse(value[i].questionID!));
          safetyDescs.add(value[i].questionDesc!);

          if (mwYesCheck == "1") {
            yesChecked.add(true);
            updateSafetyDone(qID);
            // countInt.add(1);
          } else {
      ...
      getSafetyDone().then(
        (value) {
          countComplete = value[0].countSafetyDone!;

          if (countComplete == safetyDescs.length) {
            isCanClick = true.obs;
          } else {
  ...
  Future initIDTemp() async {
    ...
    if ((prefs.getString('safetyDone')) == "completed") {
      isCanClick = true.obs;
    } else {
      isCanClick = false.obs;
    }
  ...
  Future<List<SafetyModel>> fetchSafetyGroup() async {
    SharedPreferences prefs = await SharedPreferences.getInstance();
    ...
    if (langaugeText == "TH") {
      final response = await http.get(Uri.parse(API.getSafetyGroup));
      if (response.statusCode == 200) {
        final List result = json.decode(response.body);
        return result.map((e) => SafetyModel.fromJson(e)).toList();
      } else {
        throw Exception('Failed to load data');
      }
    } else {
      final response = await http.get(Uri.parse(API.getSafetyGroupEN));
      if (response.statusCode == 200) {
        final List result = json.decode(response.body);
        return result.map((e) => SafetyModel.fromJson(e)).toList();
      } else {
  ...
  Future<List<SafetyDoneModel>> getSafetyDone() async {
    try {
      final response = await http.get(Uri.parse(API.countSafetyDone));
      if (200 == response.statusCode) {
        List<SafetyDoneModel> list = parseResponse(response.body);
        return list;
      } else {
        return <SafetyDoneModel>[];
      }
    } catch (e) {
      return <SafetyDoneModel>[];
    }
  ...
  static List<SafetyDoneModel> parseResponse(String responseBody) {
    final parsed = json.decode(responseBody).cast<Map<String, dynamic>>();
    return parsed
        .map<SafetyDoneModel>((json) => SafetyDoneModel.fromJson(json))
        .toList();
  }
  ...
  Future updateSafetyDone(String questionID) async {
    final response = await http.post(
      Uri.parse(API.updateSafetyDone),
      body: {
      ...
      child: SafeArea(
        child: Scaffold(
          ...
          appBar: AppBar(
            ...
            title: Text(langaugeText == "TH".obs ? SafetyTextTH : SafetyTextEN),
            centerTitle: true,
          ),
          body: Column(
            children: [
                                ...
                                Expanded(
                                  ...
                                  child: CheckboxListTile(
                                    onChanged: (value) async {
                                      var id = '$idtemp${safetyID[index]}';
                                      var qID = safetyID[index].toString();
                                      ...
                                      await updateSafetyDone(qID);

                                      await getSafetyDone().then(
                                        (value) {
                                          // print(
                                          //     'value: ${value[0].countEquipmentDone}');

                                          countComplete =
                                              value[0].countSafetyDone!;
                ...
                child: Obx(() => ElevatedButton(
                      ...
                      onPressed: () async {
                        if (isCanClick == true.obs) {
                          ...
                          prefs.setString('safetyDone', "completed");
