...
import 'package:management_walk/features/models/count_sectionStoreDone_model.dart';
...
import '../models/sectionStoreGroup_model.dart';
...
class SectionStoreScreen extends StatefulWidget {
  const SectionStoreScreen({Key? key}) : super(key: key);

  @override
  State<SectionStoreScreen> createState() => _SectionStoreScreenState();
}

class _SectionStoreScreenState extends State<SectionStoreScreen> {
  var sectionStoreID = <int?>[];
  var sectionStoreDescs = <String>[];
  var yesChecked = <bool>[];
  var noChecked = <bool>[];
  ...
  void initState() {
    ...
    fetchSectionStoreGroup().then((value) async {
      for (int i = 0; i < value.length; i++) {
        ...
        setState(() {
          sectionStoreID.add(int.parse(value[i].questionID!));
          sectionStoreDescs.add(value[i].questionDesc!);

          if (mwYesCheck == "1") {
            yesChecked.add(true);
            updateSectionStoreDone(qID);
            // countInt.add(1);
          } else {
      ...
      getSectionStoreDone().then(
        (value) {
          // print('value: ${value[0].countEquipmentDone}');

          countComplete = value[0].countSectionStoreDone!;

          if (countComplete == sectionStoreDescs.length) {
            isCanClick = true.obs;
          } else {
  ...
  Future initIDTemp() async {
    ...
    if ((prefs.getString('sectionStoreDone')) == "completed") {
      isCanClick = true.obs;
    } else {
      isCanClick = false.obs;
    }
  ...
  Future<List<SectionStoreModel>> fetchSectionStoreGroup() async {
    SharedPreferences prefs = await SharedPreferences.getInstance();
    ...
    if (langaugeText == "TH") {
      final response = await http.get(Uri.parse(API.getSectionStoreGroup));

      if (response.statusCode == 200) {
        final List result = json.decode(response.body);
        return result.map((e) => SectionStoreModel.fromJson(e)).toList();
      } else {
        throw Exception('Failed to load data');
      }
    } else {
      final response = await http.get(Uri.parse(API.getSectionStoreGroupEN));
  ...
  Future<List<SectionStoreDoneModel>> getSectionStoreDone() async {
    try {
      final response = await http.get(Uri.parse(API.countSectionStoreDone));
      if (200 == response.statusCode) {
        List<SectionStoreDoneModel> list = parseResponse(response.body);
        return list;
      } else {
        return <SectionStoreDoneModel>[];
      }
    } catch (e) {
      return <SectionStoreDoneModel>[];
    }
  ...
  static List<SectionStoreDoneModel> parseResponse(String responseBody) {
    final parsed = json.decode(responseBody).cast<Map<String, dynamic>>();
    return parsed
        .map<SectionStoreDoneModel>(
            (json) => SectionStoreDoneModel.fromJson(json))
        .toList();
  }
  ...
  Future updateSectionStoreDone(String questionID) async {
    final response = await http.post(
      Uri.parse(API.updateSectionStoreDone),
      body: {
      ...
      child: SafeArea(
        child: Scaffold(
          ...
          appBar: AppBar(
            ...
            title: Text(langaugeText == "TH".obs
                ? sectionStoreTextTH
                : sectionStoreTextEN),
            centerTitle: true,
          ),
            ...
            children: [
              Flexible(
                ...
                child: ListView.builder(
                  itemCount: sectionStoreDescs.length,
                          ...
                          children: [
                            Row(
                              children: [
                                Expanded(
                                  flex: 3,
                                  child: Text(sectionStoreDescs[index]),
                                ),
                                Expanded(
                                    flex: 1,
                                ...
                                Expanded(
                                  ...
                                  child: CheckboxListTile(
                                    onChanged: (value) async {
                                      var id =
                                          '$idtemp${sectionStoreID[index]}';
                                      var qID =
                                          sectionStoreID[index].toString();
                                      ...
                                      await updateSectionStoreDone(qID);

                                      await getSectionStoreDone().then(
                                        (value) {
                                          // print(
                                          //     'value: ${value[0].countEquipmentDone}');

                                          countComplete =
                                              value[0].countSectionStoreDone!;

                                          if (countComplete ==
                                              sectionStoreDescs.length) {
                                            isCanClick = true.obs;
                                          } else {
                                            isCanClick = false.obs;
                                          }
                ...
                child: Obx(() => ElevatedButton(
                      ...
                      onPressed: () async {
                        if (isCanClick == true.obs) {
                          ...
                          prefs.setString('sectionStoreDone', "completed");
